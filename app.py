...
def registration():
    if request.method == 'GET':
        ...
        if first_name != '' and last_name != '' and email != '' and address !=  '' and city != 'Select City' and password != '' '' and gender != '':
            register = query_fetch_data(
                'Insert into Registration (first_name, last_name, email, address, city, password, gender) values(?,?,?,?,?,?,?)',
                (first_name, last_name, email, address, city, password, gender), "POST")
            if register:
                return render_template('login.html')
            else:
...
def dashboard():
    ...
    if 'mail' in session:
        return render_template('dashboard.html', products=products, categoryList=categoryList, register=register, cart=cart)
    else:
...
def details_products(id):
    ...
    product_id = id
    if 'mail' in session:
        return render_template('details.html', product=products[int(id) - 1], categoryList=categoryList, register=register, cart=cart)
    else:
...
def buy_product():
    ...
    if request.method == 'GET':
        ...
        return render_template('cart.html', cart=cart, price=total_price, register=register, categoryList=categoryList)
    elif request.method == 'POST':
        for item in cart:
            total_price = total_price + float(item['price'].split('$')[1])
...
def user_profile():
    ...
    print(user_profile)
    if 'mail' in session:
        return render_template('user_profile.html', user=user_profile, register=register, categoryList=categoryList)
    else:
